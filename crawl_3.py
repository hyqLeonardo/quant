...
def json_to_df(resp_json, column, csv_file):
	...
	for item in resp_json['announcements']:
		info_list = list()
		info_list.append(str(item['secCode']))	# code
...
def cninfo_http_post(start_date):
	...
	while pn:
		...
		time.sleep(0.1)
		request = urllib.request.Request(url, jdata, values)
...
def cninfo_to_df(days, start_date, column):
	...
	with open(csv_file, 'w') as csv_w:
		wr = csv.writer(csv_w, quoting=csv.QUOTE_ALL)
		wr.writerow(['股票代码'.encode('utf-8'), '公告标题'.encode('utf-8'), '公告链接'.encode('utf-8'), '日期'.encode('utf-8')])
...
if __name__ == '__main__':
	...
	start_date = '2017-06-11' # start date
	days = 4	# days to crawl
